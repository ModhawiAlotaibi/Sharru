...
#if a host is detached/attached, then this method will be called
def handle_port_status( event ):
    ...
    if event.added:
        ...
        #use IP address of root controller, with the root 
        #controller IP, same as for other cases
        s.connect(('127.0.0.1', 6670))
        REQ = action + str(event.dpid) + str(event.port)
        s.sendall(REQ)
        data = s.recv(1024)
        if( data == REQ ):
          #Add flow modification code here.
          #actions = []
          #actions.append(of.ofp_action_dl_addr.set_dst(mac))
        ...
        print "message from controller %s" % data
        s.close()
    elif event.deleted:
        action = "removed"
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
def launch (fakeways="", arp_for_unknowns=None):
  ...
  #when controller launched, when a OF_PORT_STATUS happens 
