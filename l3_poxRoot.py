...
#code for manipulation of data structure
current_controller_id = 1
switchPairToController = {
1:{ 1:1, 2:2 ,3:3, 5:4 },
2:{ 1:1, 2:2 ,3:3, 5:4 },
3:{ 1:1, 2:2 ,3:3, 5:4 },
4:{ 1:1, 2:2 ,3:3, 5:4 },
...
HOST_SWITCHIP_1 = "10.0.0.1"
originIntfToHost = { "s1-eth3":"h1" }
hostToSwitch = { "h1":1 }
...
def handle_port_status( event ):
    log.info("inside sharru")
    dpid = event.connection.dpid
    inport = event.port
    if event.added:
        action = "added"
...
#when a sub-controller requests for the connection this 
#method is called
def root_controller_listen():
  ssock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
  #binds the connection to the sub-controllers
  ssock.bind(('127.0.0.1', 6670))
  ssock.listen(4) #increase the numbers based on the number of subcontrollers
  while 1:
    conn, addr = ssock.accept()
    print "connection %s address %s" % (conn, addr) 
    while 1:
      data = conn.recv(1024) 
      if not data:
         break
      print "message received at root %s" % (data)
      t1 = time.clock()
      print "root manipulation time", t1
      conn.sendall(data)
    conn.close()
  ssock.close()
...
def launch (fakeways="", arp_for_unknowns=None):
  ...
  core.openflow.addListenerByName("PortStatus", handle_port_status)
  #This thread keeps the root controller listening for other sub-controllers
  threading.Thread(target=root_controller_listen).start()
